...
//!
//! [`Filters`] builds a `WHERE` body; [`EscapeLike`] is a small formatting
//! helper for `LIKE` clauses. Both take column names from the calling query
//! (never user input) since those names are interpolated verbatim into the
//! generated SQL.
